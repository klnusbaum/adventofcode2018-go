...
func analyze(serialNumber int) error {
	...
	x, y := largest3x3(powerLevels)
	fmt.Printf("Most power 3x3 grid is: %d,%d\n", x, y)
...
func largest3x3(powerLevels [][]int) (int, int) {
	largestX := -1
	largestY := -1
	largestPower := 0
	for i := 0; i < 297; i++ {
		for j := 0; j < 297; j++ {
			power := get3x3Power(powerLevels, i, j)
...
func get3x3Power(powerLevels [][]int, i, j int) int {
	power := 0
	for ii := i; ii < i+3; ii++ {
		for jj := j; jj < j+3; jj++ {
